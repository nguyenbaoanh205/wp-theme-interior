...
foreach ( $shortcodes as $shortcode ) {
	...
	add_shortcode( 'itcthemes_' . $shortcode, 'itcthemes_' . $shortcode );
}
 ...
 * @since    1.0
 */
if ( ! function_exists( 'itcthemes_pre_shortcode' ) ) {
	function itcthemes_pre_shortcode( $shortcode_content ) {
		$shortcodes = 'gallery, image_banner';
		$shortcodes = explode( ',', $shortcodes );
		$shortcodes = array_map( 'trim', $shortcodes );
	...
	add_filter( 'widget_text', 'itcthemes_pre_shortcode', 7 );
	add_filter( 'the_content', 'itcthemes_pre_shortcode', 7 );
}
