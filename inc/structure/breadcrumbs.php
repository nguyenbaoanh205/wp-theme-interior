...
$home      = __('Trang chủ', 'itcthemes'); // text for the 'Home' link
$blog      = __('Tin mới nhất', 'itcthemes'); // text for the 'Home' link
$shop      = __('Sản phẩm', 'itcthemes'); // text for the 'Home' link
...
if (is_front_page()) {
	...
	if (is_category()) {
		...
		if (get_post_type() != 'post') {
			...
			if ($post_type_object) {
				$slug = isset($post_type_object->rewrite['slug']) ? $post_type_object->rewrite['slug'] : '';
				$label = isset($post_type_object->labels->singular_name) ? $post_type_object->labels->singular_name : '';
				echo '<li><a href="' . esc_url(home_url('/' . $slug . '/')) . '">' . $label . '</a></li>';
			}
			echo ' ' . $before . get_the_title() . $after;
		} else {
		...
		if ($post_type_object && isset($post_type_object->labels->singular_name)) {
			echo $before . $post_type_object->labels->singular_name . $after;
		}
	...
	if (get_query_var('paged')) {
		...
		echo $before . __('Page', 'itcthemes') . ' ' . get_query_var('paged') . $after;
		if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ')';
	}
